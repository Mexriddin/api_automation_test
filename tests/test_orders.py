...
class TestOrder(BaseTest):
    ...
    def test_create_order(self, user, game):
        order = self.api_orders.create_order(user_uuid=user["model"].uuid, game_id=game.uuid)
        assert order.status == "open"
        assert order.items[0].item_uuid == game.uuid
        assert order.user_uuid == user["model"].uuid

    @allure.title("Get an order")
    def test_get_order(self, user, game):
        new_order = self.api_orders.create_order(user_uuid=user["model"].uuid, game_id=game.uuid)
    ...
    def test_list_orders(self, user):
    ...
    def test_update_order_status(self, user, game):
        order = self.api_orders.create_order(user_uuid=user["model"].uuid, game_uuid=game.uuid)
    ...
    def test_delete_order(self, user, game):
        order = self.api_orders.create_order(user_uuid=user["model"].uuid, game_uuid=game.uuid)
